...
class Manus:
    ...
    def get_time_diff(self):
        ...
        year_diff = int(time_now.split(' ')[0].split('-')[0]) - int(self.modify_time.split(' ')[0].split('-')[0])
        month_diff = int(time_now.split(' ')[0].split('-')[1]) - int(self.modify_time.split(' ')[0].split('-')[1])
        day_diff = int(time_now.split(' ')[0].split('-')[2]) - int(self.modify_time.split(' ')[0].split('-')[2])
        hour_diff = int(time_now.split(' ')[0].split(':')[0]) - int(self.modify_time.split(' ')[0].split(':')[0])
        # min_diff = int(time_now.split(' ')[0].split(':')[1]) - int(self.modify_time.split(' ')[0].split(':')[1])
        # sec_diff = int(time_now.split(' ')[0].split(':')[2]) - int(self.modify_time.split(' ')[0].split(':')[2])
        if year_diff > 1:
