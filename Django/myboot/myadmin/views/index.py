...
def do_signin(request):
    try:
        ...
        if Ad.u_status != 0:
            context = {"info": "login failed! Administrator account not available, please contact with Super-Administrator",
                       "status": 0,
                       }
...
def signup(request):
    return render(request, 'myadmin/Signup.html')
...
def signup_check(request):
    try:
        ...
        return render(request, 'myadmin/Signupcheck.html', context)
    except Exception as err:
        print('***************')
        print(err)
